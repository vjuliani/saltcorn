...
- **`CapDBRead` é a única capacidade implementada** — um callback de LEITURA genérico, resolvido reaproveitando `internal/records.Rows` com a MESMA autorização por papel de todo o resto do backend (GO-008/012/015) — nenhuma credencial de banco cruza para o processo Node. Escrita a partir de uma expressão fica fora de escopo (relatório de GO-004 §5: não prototipada, precisa de decisão própria de idempotência) — classificada como bloqueadora até GO-023 ou depois.
- **`ExpressionCapability = "plugins.expr"`** é o nome de capacidade que uma integração futura (GO-023) deve usar com `internal/platform/cutover.RequireOwnership`/`SwitchOwner` — reaproveitando o MESMO mecanismo já em produção desde GO-009 (`tables.records`/`tables.schema`/`tables.views`), não um mecanismo por-plugin novo. Enquanto nenhuma chamada a `SwitchOwner` mencionar essa capacidade, `cutover.OwnerOf` devolve `OwnerLegacy` para qualquer tenant — o critério de aceite "plugin transacional incompatível mantém operação integral no legado" já vale por construção, provado em `internal/pluginhost/cutover_test.go`.
