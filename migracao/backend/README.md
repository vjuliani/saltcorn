...
- **`internal/views` não tem runtime de renderização** — armazena `template` como uma string de intenção não interpretada e `configuration` como o JSON de layout do builder, opacamente. Renderizar uma view (interpretar `template`/`configuration` e produzir HTML) é GO-020 ("Portar runtime de views e páginas"), que não existe como dependência desta tarefa nem foi antecipado aqui.
...
- **Rotas novas:** `POST /v1/tenants/{tenant}/tables`, `POST /v1/tenants/{tenant}/tables/{table}/fields`, `POST /v1/tenants/{tenant}/views`, `GET /v1/tenants/{tenant}/views/{id}`, `PATCH /v1/tenants/{tenant}/views/{id}` — todas atrás de `cutover.RequireOwnership` com a capacidade correspondente.
