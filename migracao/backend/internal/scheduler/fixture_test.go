...
// fixture_test.go, internal/platform/lease/lease_test.go.
package scheduler
...
func schedulerFixture(t *testing.T) (db *database.DB, tenant tenancy.Tenant) {
	t.Helper()
	db = testDB(t)
	ctx := context.Background()
	tenant = tenancy.Tenant(fmt.Sprintf("scheduler_test_%s", sanitizeForSchema(t.Name())))
