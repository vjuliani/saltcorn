...
Migrar incrementalmente para um **monólito modular em Go**, com **CQRS lógico**, **BFF web em Go** e reaproveitamento inicial do frontend JavaScript/React. PostgreSQL será o primeiro banco suportado pelo novo backend. SQLite, mobile e extensões continuam no escopo de conclusão, mas entram em etapas posteriores.

CQRS separa os caminhos de escrita e leitura; BFF (Backend for Frontend) adapta os serviços às necessidades de uma interface. Ambos podem começar no mesmo processo Go. Separar responsabilidades não exige separar serviços.

A conversão não é tradução automática de TypeScript: Saltcorn executa aplicações definidas por metadados e extensões. O maior risco é alterar essa semântica. O objetivo inicial é backend Go com compatibilidade controlada; remover completamente Node do servidor exige migrar ou substituir todos os plugins e scripts em uso. O frontend continuará usando tecnologias do navegador.

As tarefas executáveis estão em [TASKS.md](TASKS.md). Premissas a validar na fase 0: aplicações prioritárias, plugins instalados, volume de dados, SLOs, equipe, exigência de SQLite/mobile e necessidade de servidor sem Node. Não há inventário de produção neste checkout.
...
```mermaid
flowchart TD
  UI[Admin e builder React / aplicações web] --> EDGE[Entrada HTTP e roteamento da migração]
  MOBILE[Mobile] --> EDGE
  ...
  subgraph GO[Monólito modular Go]
    BFF --> Q[Queries: consultas e DTOs]
    BFF --> C[Commands: validação e regras]
    API --> Q
    API --> C
...
Banco compartilhado somente durante a transição, com proprietário único de escrita por capacidade/tenant e coordenação explícita de alterações de schema. O diagrama não autoriza Node e Go a modificar livremente o mesmo agregado. O host JS não recebe credenciais irrestritas do banco.
...
Estrutura sugerida, ainda não criada: `go/cmd/{server,worker,cli}`, `go/internal/{identity,metadata,records,views,automation,extensions,files,sync}`, `go/internal/platform/{database,http,telemetry}` e `contracts/`. Cada domínio pode ter `commands`, `queries` e adapters sem um framework genérico obrigatório.
...
Adotar um módulo BFF web Go para compor bootstrap do editor, permissões de interface, metadados e dados de telas. Ele cuida de sessão/cookies, CSRF, paginação e erros voltados à UI; domínio revalida autorização em todo comando/query. Não acessa tabelas diretamente nem concentra regras de negócio. Integrações usam API pública versionada; mobile pode ganhar BFF próprio quando contratos e uso justificarem.

No início, BFF e backend compartilham processo e chamadas de aplicação. Extração para serviço independente depende de necessidades de escala, equipe ou ciclo de release. Um BFF separado acrescenta operação e latência; a especialização por cliente é o benefício a demonstrar. [Referência BFF](https://learn.microsoft.com/en-us/azure/architecture/patterns/backends-for-frontends).
...
Plugins JS exigem classificação: tipos, fieldviews, ações, views, autenticação, hooks e dependências Node. Host temporário por RPC com capacidades explícitas, limites de memória/CPU/tempo e isolamento de processo/OS; simples uso de VM não constitui toda a fronteira de segurança. Plugins que dependem de transações ou objetos internos ficam no caminho legado até adaptação. Backend 100% Go só é declarado depois de retirar esse host para o escopo contratado.
...
| F1 Fundação | Estrutura Go, contratos, identidade e tenancy | Isolamento e políticas equivalentes demonstrados |
| F2 Dados/CQRS | Metadados, queries, commands e outbox | CRUD dinâmico com transação e concorrência validado |
| F3 Experiência web | BFF, editor integrado e renderizador piloto | Criar tabela → editar view → publicar → operar registros |
...
| F6 Corte | Canário, recuperação e retirada do legado | Operação estável e dependências Node resolvidas |
...
Planejamento indicativo, sem compromisso de prazo: descoberta 2–4 semanas; fundação e fluxo vertical 6–10; piloto web operacional mais 6–10. Hipótese: 2 profissionais Go, 1 frontend, 1 QA/automação, apoio parcial de plataforma e alguém com conhecimento Saltcorn. Intervalos são hipóteses de calendário e não medições; paridade completa de plugins/mobile requer reestimativa após F0. Usar tamanhos relativos do backlog para refinamento, sem convertê-los automaticamente em dias.
...
Conclusão da migração: todas as capacidades acordadas funcionam em Go, dados e configurações foram reconciliados, recuperação foi testada, operação está documentada e nenhuma dependência Node permanece no servidor se esse for o requisito aprovado de produto. Uso de bridge significa migração híbrida ainda em andamento.
