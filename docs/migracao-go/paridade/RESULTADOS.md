...
| CAP-073 — Ação `form_action` (mecanismo real do "Salvar" em toda view Edit) | sim | NÃO LISTADA ANTERIORMENTE | `base-plugin/actions.ts` | Bloqueia diretamente GO-039 (Edit sem isto não salva dado algum) — escopo emendado em GO-039 |
| CAP-074 — Ação `navigate` (redirecionamento pós-ação) | sim | NÃO LISTADA ANTERIORMENTE | `base-plugin/actions.ts` | Mecanismo onipresente em botões de view/página — escopo emendado em GO-039 |
...
| CAP-085 — Nós de layout além de `field`/List (join_field, view_link, action, aggregation, filtros, ~19 nós de página) | sim | DETALHAMENTO de CAP-035/038 | `saltcorn-types/base_types.ts:112-169`, `internal/views/render.go` | Builder legado (reaproveitado, não Craft.js) continua produzindo todos os ~36 tipos; runtime novo só renderiza `field`/List — detalhar em GO-039 |
