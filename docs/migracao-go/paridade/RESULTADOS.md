...
| CAP-075 — Versionamento de linha / histórico de registro (table history) | sim | NÃO LISTADA ANTERIORMENTE | `models/table.ts`, `routes/list.ts` (`/_versions`, `/_restore`) | Feature de produto (audit trail + reverter linha) sem catálogo Go — ver GO-045 |
...
| CAP-082 — Internacionalização (i18n) da interface, incl. tradução assistida por LLM | sim | NÃO LISTADA ANTERIORMENTE | `app.js` (lib `i18n`), `routes/infoarch.ts` (`/localizer/*`), `cli/dev/translate.js` | Capacidade transversal de produto sem nenhuma menção — ver GO-047 |
