...
| --- | --- | --- | --- | --- |
| CAP-071 — Administração de usuário e servidor (impersonação, SSL/Let's Encrypt, permissões por tabela) | sim (segurança) | NÃO LISTADA ANTERIORMENTE | `packages/server/auth/admin.ts` (1524L, roteado fora de `routes/`) | Nenhum equivalente Go; superfície de segurança de alto risco (impersonação de admin, emissão de certificado) sem avaliação nenhuma — ver GO-044 |
| CAP-072 — Administração de roles (CRUD de role customizada, restrição de auth/layout/push por role) | não | NÃO LISTADA ANTERIORMENTE | `packages/server/auth/roleadmin.ts` (441L) | Go só tem papéis fixos por constante; sem equivalente — ver GO-044 |
