...
| Motor de expressões JS (fórmulas de usuário) | `models/expression.ts` (1321 linhas); `vm2` no servidor (linha 7,30,42-53), `vm.runInNewContext`/`vm-browserify` no bundle mobile (linha 11,51) | Fórmulas calculadas, `ownership_formula`, condições de trigger | `fixtures.ts` (`run_js_code` triggers) | **Compatibilidade Node temporária (parcial) — refinado por GO-004/GO-022** | Classificação ATUALIZADA: antes de GO-004, esta linha era "Bloqueador" simples. O protótipo de GO-004 (`docs/migracao-go/prototipos/GO-004-fronteira-rpc/`) e o host de produção de GO-022 (`migracao/packages/pluginhost` + `migracao/backend/internal/pluginhost`) provam que **fórmulas puras sobre `row`/`user` e um callback de LEITURA genérico SÃO viáveis** via um host isolado (processo separado, sem `vm2` — módulo `vm` nativo do Node, isolamento real é o PROCESSO: `--max-old-space-size` + timeout/kill do lado Go, testado por regressão deliberada). Continuam **bloqueadoras**, sem solução nesta tarefa, três classes concretas (critérios de triagem, não teóricos): (1) fórmula que serializa uma função com closure do ambiente de origem (nenhuma fronteira de processo transporta ambiente léxico — GO-004 caso #9); (2) fórmula/expressão que ESCREVE dados (callback de escrita não prototipado nem implementado — decisão própria de idempotência pendente, GO-023 ou depois); (3) fórmula que referencia um singleton de domínio (`Table`/`File`/`View`/`User`) diretamente, sem passar pelo canal de callback explícito (vira `undefined` silenciosamente, GO-004 caso #6) — toda expressão herdada que hoje faz isso precisa ser identificada antes de qualquer corte real da capacidade `plugins.expr` (ver `internal/pluginhost.ExpressionCapability`). `vm2` continua descontinuado e não é usado no host de produção (ver ADR-0005 e nota de escopo 2 de `docs/migracao-go/execucoes/GO-022.md`) |
